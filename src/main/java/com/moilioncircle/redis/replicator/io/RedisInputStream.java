...
public class RedisInputStream extends InputStream {
    ...
    protected List<RawByteListener> rawByteListeners = Collections.emptyList();
    ...
    public synchronized void setRawByteListeners(List<RawByteListener> rawByteListeners) {
        this.rawByteListeners = rawByteListeners == null ? Collections.emptyList() : rawByteListeners;
    }

    private void notify(int len) {

        if (rawByteListeners.size() == 1) {
            rawByteListeners.get(0).handle(len);
        } else if (rawByteListeners.size() != 0) {
